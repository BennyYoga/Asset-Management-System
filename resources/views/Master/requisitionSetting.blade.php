                        ...
                        <div class="card-style mb-30">
                            <div class="row mt-3">
                                ...
                                <div class="col-lg-5">
                                    ...
                                    <input type="hidden" id="IdJabatan" value="{{$id}}">
                                    <div class="select-style-1">
                                        <label>Approval Order</label>
                                        <div class="select-position">
                                            <select name="ApprovalOrder" id="ApprovalOrder" required>
                                                @php $i=1; @endphp
                                                @for ($i=1; $i<=count($approvalOrders) ; $i++) <option value="{{ $i }}">Approval #{{$i }}</option>
                                                    @endfor
                                                    <option value="{{ $i }}" selected>Approval #{{$i }}</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="select-sm select-style-1" id="approver-level-form">
                                        ...
                                        <div class="select-position input-tags">
                                            <select class="js-example-basic-single form-" id="tags" value="" multiple name="Approver[]">
                                                @foreach($allApprovers as $approver)
                                                <option value="{{ $approver['RoleId'] }}">{{ $approver['RoleName'] }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
...
<script type="text/javascript">
    ...
    $('#ApprovalOrder').change(function() {
        var valuOrder = $('#ApprovalOrder').find(":selected").val();
        var IdJabatan = $('#IdJabatan').val();
        var dataApprover = null;
        $.ajax({
            type: 'GET',
            url: `/master/requisition/ApprovalOrder/${valuOrder}/${IdJabatan}`,
            success: function(data) {
                $('#tags').val(null);
                $("#tags").val(data).change();
                // data.forEach(element => {
                //     $('#tags').val(null);
                //     // $('#tags').append($(`<option value=${element.ApproverId} class="ajaxset" selected>${element.ApproverName}</option>`));
                // });
            },
